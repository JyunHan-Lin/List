...
public class Coffee {
	...
	//也可以用list.of(), 但之後就不能再新增了
	
	
	// 類別資料初始化區段
	static {
		// 初始化規則
		rules.add(new CoffeeRule("濃郁的咖啡", "牛奶的比例遠高於咖啡，味道偏向牛奶。", 
		...
		rules.add(new CoffeeRule("標準卡布奇諾", "牛奶與咖啡的比例恰到好處，典型的卡布奇諾。", 
				(milk, coffee) ->milk.doubleValue() == coffee.doubleValue())); // 也可以寫成: milk.equals(coffee)
		rules.add(new CoffeeRule("濃縮咖啡", "咖啡的比例遠高於牛奶，口感濃烈，偏向純咖啡。", 
				(milk, coffee) ->coffee >= 3*milk));
		rules.add(new CoffeeRule("普通的咖啡", "牛奶與咖啡的比例較為普通，適合日常飲用。", 
				(milk, coffee) -> true));
	}
